...
function warnIfNotCurrentlyActingEffectsInDEV(fiber) {
  {
    if (warnsIfNotActing === true && (fiber.mode & StrictLegacyMode) !== NoMode && ReactCurrentActQueue.current === null && // Our internal tests use a custom implementation of `act` that works by
...
function warnIfNotCurrentlyActingUpdatesInDEV(fiber) {
  {
    if (warnsIfNotActing === true && executionContext === NoContext && ReactCurrentActQueue.current === null && // Our internal tests use a custom implementation of `act` that works by
    // mocking the Scheduler package. Disable the `act` warning.
    // TODO: Maybe the warning should be disabled by default, and then turned
    // on at the testing frameworks layer? Instead of what we do now, which
    // is check if a `jest` global is defined.
...
exports.flushSyncWithoutWarningIfAlreadyRendering = flushSyncWithoutWarningIfAlreadyRendering;
